import {
  ...
  export const appwriteConfig = {
    ...
    ratingId:"66f43d3b0017dc2d16a6",
    bucketId:"66e6853c003348edcf9c"
  };
  ...
  export async function getUpdate(data) {
  ...
  export async function getLatestPosts() {
    ...
    export async function getUserBooking(userId) {
      try {
        const posts = await databases.listDocuments(
          ...
          [
            Query.and([
              Query.equal("accountId", userId) ,  // First condition
              Query.equal("status", "PENDING") , // Second condition
            ])
          ]
    ...
    export async function createRating(form){
      try{     
        const newRating = await databases.createDocument(
          appwriteConfig.databaseId,
          appwriteConfig.ratingId,
          ID.unique(),
          {
            accountId: form.accountId,
            rating: form.rating, 
    ...
    export async function updateProfile (url) { 
      ...
      const query = 
      [Query.equal('accountId', currentAccount.$id)]
      const documents = await databases.listDocuments(appwriteConfig.databaseId, appwriteConfig.userCollectionId, query);
      ...
      databases.updateDocument(appwriteConfig.databaseId, appwriteConfig.userCollectionId, documentId, {avatar:url})
      .then(response => {
          console.log('Document updated successfully:', response);
      })
      .catch(error => {
    ...
    export async function sendEmail (emails) {
      const to = emails; // Recipient email
